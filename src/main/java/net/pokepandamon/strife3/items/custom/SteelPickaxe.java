...
public class SteelPickaxe extends CustomPickaxe {
    public static final double attackDamage = 4.0;
    public static final double attackSpeed = -3.5;
    ...
    public SteelPickaxe(String itemType, ToolMaterials toolMaterials, Settings settings) {
        super(itemType, toolMaterials, settings);
    }

    @Override
    public boolean onStackClicked(ItemStack stack, Slot slot, ClickType clickType, PlayerEntity player){
        stack.addEnchantment(player.getWorld().getRegistryManager().get(RegistryKeys.ENCHANTMENT).getEntry(EFFICIENCY).get(), 1);
