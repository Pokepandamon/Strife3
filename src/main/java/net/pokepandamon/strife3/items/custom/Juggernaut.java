...
public class Juggernaut extends CustomArmor implements GeoItem {
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    public Juggernaut(String itemType, RegistryEntry<ArmorMaterial> armorMaterial, Type type, Settings properties) {
        super(itemType,armorMaterial, type, properties);
    }

    @Override
    public boolean onStackClicked(ItemStack stack, Slot slot, ClickType clickType, PlayerEntity player){
        stack.addEnchantment(player.getWorld().getRegistryManager().get(RegistryKeys.ENCHANTMENT).getEntry(PROTECTION).get(), 3);
